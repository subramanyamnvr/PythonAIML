...
MCP server + optional **Streamlit** UI for a **local-first study workflow**: an allow-listed **Brain folder** (Markdown), **semantic memory** (ChromaDB on disk + OpenAI embeddings), **public web fetch**, **SQLite scratch notes**, optional **Pushover**, and **time** in any IANA timezone. Built with **FastMCP** (aligned with the `06-MCP` course—**not** a trading stack).
...
```bash
cd 06-MCP/community_contributions/asket-mcp
```

If you are **already inside** the `06-MCP` folder:
