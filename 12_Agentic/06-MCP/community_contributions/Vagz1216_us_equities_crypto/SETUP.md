...
This folder is a **Week 6 community contribution**. Clone the **repo** (not this folder alone). MCP Python servers and account wiring for this variant live **in this directory**; `database.py`, `tracers.py`, and `util.py` still come from `06-MCP/` on `sys.path`.
...
You can also use the class-wide reset (same DB), now aligned with multi-asset in `06-MCP/reset.py`:
...
(from this directory, or use the full path under `06-MCP` as above)
...
From `06-MCP/` only: `database.py`, `tracers.py`, `util.py`.
