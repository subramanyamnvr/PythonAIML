...
```json
{
  "mcpServers": {
    "ship-logistics": {
      ...
      "args": ["/absolute/path/to/agents/06-MCP/community_contributions/abdussamadbello_ship_logistics/server.py"],
      "cwd": "/absolute/path/to/agents/06-MCP/community_contributions/abdussamadbello_ship_logistics"
    }
  }
}
```
...
```bash
cd 06-MCP/community_contributions/abdussamadbello_ship_logistics
...
Uses `mcp` with `FastMCP` (same stack as the course `06-MCP` labs). Install from the course environment / `requirements.txt` at repo root.
