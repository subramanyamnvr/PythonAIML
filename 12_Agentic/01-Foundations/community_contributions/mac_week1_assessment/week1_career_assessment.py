...
Run from repo root:
  uv run python 01-Foundations/community_contributions/mac_week1_assessment/week1_career_assessment.py

Requires `me/linkedin.pdf` and `me/summary.txt` under `01-Foundations/me/` (course default).
"""
...
def _find_me_dir() -> Path:
    ...
    for base in [here, *here.parents]:
        ...
        if candidate.is_dir() and (candidate / "summary.txt").is_file():
            return candidate
        # contribution folder: ../../me from 01-Foundations/community_contributions/x/
        alt = base.parent / "me"
        if alt.is_dir() and (alt / "summary.txt").is_file():
            return alt
    raise FileNotFoundError(
        "Could not find 01-Foundations/me with summary.txt — place your profile files there."
    )
