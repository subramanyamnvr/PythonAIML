...
**Still loaded from `06-MCP/`** via `sys.path`: `database.py`, `tracers.py`, `util.py`.

## After cloning this PR

1. Repo root: `uv sync`
2. Configure `.env` (see `SETUP.md`)
3. From `06-MCP`, reset traders (multi-asset strategies):

```bash
uv run community_contributions/Vagz1216_us_equities_crypto/reset.py
```

Or use the class entrypoint (same DB, same strategies as `06-MCP/reset.py` now):
...
```bash
cd 06-MCP/community_contributions/Vagz1216_us_equities_crypto
...
Or from `06-MCP`: `uv run community_contributions/Vagz1216_us_equities_crypto/trading_floor.py`
...
- [x] Built inside `06-MCP/community_contributions/...` (instructor path)
...
- [x] Aligned `06-MCP/reset.py` strategy text with multi-asset (equities + crypto)
