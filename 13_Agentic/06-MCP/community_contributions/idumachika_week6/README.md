...
Submission for **Week 6 (Model Context Protocol)** aligned with **`06-MCP/2_lab2.ipynb`**.
...
```bash
uv run python 06-MCP/community_contributions/idumachika_week6/date_agent_demo.py
```

This starts the MCP server as a subprocess via `uv run` with `cwd` set to **`06-MCP/`** so dependencies resolve like the course `accounts_server` example.
...
Add under `06-MCP/community_contributions/` and open a PR to [ed-donner/agents](https://github.com/ed-donner/agents).
